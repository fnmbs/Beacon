...
export default function Paths() {
  ...
  const save = (data) => {
    toast_(modal?.id ? "Path updated" : "Path created");
    setModal(null);
  };
  ...
  const filtered = search
    ? paths.filter((p) =>
        p.from_name?.toLowerCase().includes(search.toLowerCase()) ||
    ...
    <div className="flex flex-col min-h-screen" style={{ background: "#fff", fontFamily: "'Instrument Sans', sans-serif" }}>
      ...
      <header className="flex items-center justify-between px-8 shrink-0" style={{ height: "52px", borderBottom: "1px solid #000" }}>
        <div className="flex items-center gap-3">
          <span style={{ fontSize: "11px", fontWeight: 500, letterSpacing: "0.06em", color: "#000" }}>Paths</span>
          <span style={{ fontSize: "10px", color: "#000", opacity: 0.35 }}>{totalPaths}</span>
          <span style={{ color: "#ddd" }}>|</span>
          <span style={{ fontSize: "10px", color: "#000", opacity: 0.35, letterSpacing: "0.04em" }}>{totalPathsDistance}m total</span>
        </div>
        <button
      ...
      <div className="flex-1 p-8">
        ...
        <div style={{ border: "1px solid #000" }}>
          ...
          {/* Column headers */}
          <div className="grid px-5 py-3" style={{ gridTemplateColumns: "1fr 28px 1fr 100px 80px", borderBottom: "1px solid #000" }}>
            {["From", "", "To", "Distance", ""].map((h, i) => (
              <div key={i} style={{ fontSize: "9px", fontWeight: 500, letterSpacing: "0.25em", textTransform: "uppercase", color: "#000", opacity: 0.35 }}>
                {h}
              </div>
            ))}
          </div>
          ...
          {!loading && error && (
            <div className="py-16 flex flex-col items-center gap-3">
              <span style={{ fontSize: "11px", color: "#000", opacity: 0.4 }}>Failed to load paths.</span>
              <button onClick={() => fetchPaths(page, limit)} style={{ fontSize: "10px", color: "#000", border: "1px solid #000", padding: "4px 12px", background: "none", cursor: "pointer" }}>
                Try again
              </button>
            </div>
          )}
          ...
          {!loading && !error && filtered.map((path, i) => (
            <div
              key={path.id}
              className="group grid px-5 py-3.5 items-center hover:bg-[#f9f9f9] transition-colors"
              style={{ gridTemplateColumns: "1fr 28px 1fr 100px 80px", borderBottom: i < filtered.length - 1 ? "1px solid #f0f0f0" : "none" }}
            >
              <span style={{ fontSize: "12px", fontWeight: 500, color: "#000", letterSpacing: "0.02em" }}>
                {path.from_name}
              </span>
              <span>
                <svg viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.2" className="w-3 h-3" style={{ color: "#000", opacity: 0.2 }}>
                  <path d="M1 6h10M7 2l4 4-4 4" />
                </svg>
              </span>
              <span style={{ fontSize: "12px", color: "#000", opacity: 0.5, letterSpacing: "0.02em" }}>
                {path.to_name}
              </span>
              <span style={{ fontSize: "12px", fontWeight: 500, color: "#000", letterSpacing: "0.02em" }}>
                {Number(path.distance_meters).toFixed(1)}
                <span style={{ fontSize: "10px", marginLeft: "2px", opacity: 0.4 }}>m</span>
              </span>
              <div className="flex gap-1.5 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                <button
                  onClick={() => setModal(path)}
                  className="hover:opacity-50 transition-opacity"
                  style={{ fontSize: "10px", color: "#000", border: "1px solid #000", padding: "3px 10px", background: "none", cursor: "pointer", letterSpacing: "0.04em" }}
                >
                  Edit
                </button>
                <button
                  onClick={() => del(path.id)}
                  className="hover:opacity-50 transition-opacity"
                  style={{ fontSize: "10px", color: "#cc0000", border: "1px solid #cc0000", padding: "3px 10px", background: "none", cursor: "pointer", letterSpacing: "0.04em" }}
                >
                  Del
                </button>
              </div>
          ...
          <div className="flex items-center justify-between px-5 py-3" style={{ borderTop: "1px solid #000" }}>
            <span style={{ fontSize: "10px", color: "#000", opacity: 0.4, letterSpacing: "0.04em" }}>
              {totalPaths > 0 ? `${(page - 1) * limit + 1}–${Math.min(page * limit, totalPaths)} of ${totalPaths}` : "No paths"}
            </span>
...
function PathModal({ initial, onClose, onSave }) {
  const [form, setForm] = useState({
    from_location_id: initial?.from_location_id || "",
    to_location_id: initial?.to_location_id || "",
    distance_meters: initial?.distance_meters || "",
  });
    ...
    >
      ...
      >
        ...
        <div className="p-6 flex flex-col gap-5">
          {[
            { label: "From", name: "from_location_id" },
            { label: "To", name: "to_location_id" },
          ].map((f) => (
            <div key={f.name}>
